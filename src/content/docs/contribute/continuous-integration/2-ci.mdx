...
:::note
**Usually, containers detects out-of-bound resource consumption automatically**. When using the [devcontainer](nf-neuro/contribute/setup#development-container)
or configuring **Nextflow** to run using `docker` or `apptainer`, tests will fail if they haven't been assigned them sufficiently.
It can be a good way to see if your workload needs more resources quickly !
:::
...
:::caution[You should not need more resources for **subworkflows** !]
Subworkflows use **stub-runs** to test their integration, but they don't run the actual workload of their modules. They thus
should not ask for more resources than what's needed to call **help commands** and create empty files.

There are definitely use-cases, so if you think otherwise, before implementing the resource request above, open a [discussion
with the maintainers !](https://github.com/scilus/nf-neuro/issues/new?template=usage_issue.md)
:::
