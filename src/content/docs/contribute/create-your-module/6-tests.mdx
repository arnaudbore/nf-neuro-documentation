...
Adding tests to your module is a critical step to ensure it works correctly and continues to do so as the codebase evolves.
The `nf-core` command automatically generates the test infrastructure when creating a module. Inside the **tests** directory of
your module, you'll find **_main.nf.test_**, which contains inputs, test cases, and assertion instructions for
[nf-test](https://www.nf-test.com), the Nextflow test framework. Open it and follow through the rest of this section.

:::note
In the **_main.nf.test_** file, define multiple test cases to cover most use-cases of your module and catch potential bugs. Be
thorough but practical with your test coverage. **A good test case should not take more than a minute to run**, though there
are exceptions.
:::
...
return values. In **nf-neuro**, test datasets are provided via the [LOAD_TEST_DATA](/api/subworkflows/load_test_data)
subworkflow (refer to [this section](/contribute/continuous-integration/1-test-data) to learn how to find packages).
Once you've selected the archives and files needed, add a `setup` section before the test section:
...
the image data itself might be unreproducible, up to few decimals.
...
Module configuration is done using a **_nextflow.config_** file. You provide it either **globally** or inside each test case,
using the `config` parameter :

<Tabs>
<TabItem label="Global configuration">
```groovy { 5 }
nextflow_process {
...
<TabItem label="Test case configuration">
```groovy { 4 }
nextflow_process {
    ...
    test("My test case") {
...
### Edit **_tests/nextflow.config_**

The **_nextflow.config_** file does not exist by default, so you will have to
...
test cases. Refer to [this section](/contribute/create-your-module/3-configuration) to
see how to scope those parameters to specific tests using `selectors`.

:::tip[Layered configuration]
**nf-test** supports layering configuration files. It applies configuration in hierarchy, from parent `config` statements
defined in the `nextflow_process` block down to overriding `config` in children `test` blocks.
:::

## Obligatory stub testing

All modules in **nf-neuro** contain a `stub` section, which requires a test case in the **_tests/main.nf.test_** file. It
may seem redundant, but those tests are essential to the **CI/CD** infrastructure, to quickly test that downstream workflows
work well without executing the module itself. We call this **functional testing**, if you contribute a subworkflow you'll
learn more about it.

To enable those tests for downstream users, create a new test case, like done previously, specifying inputs and potential
configuration. To run the module in **stub mode** add the `tag "stub"` and `options "-stub-run"` clauses before the `when`
block.
...
:::tip
It could be good to test the different inputs your module accepts, if it has optional ones. You can do that in a single
test case, by providing multiple inputs in each channels (refer above).
:::

For stubs, the only assertion needed is on the **_version.yml_** file that gets generated by the module. Here is a complete
stub test with assertion on the **_version.yml_** file :
...
**_tests/main.nf.test.snap_** in your module's directory and validate that ALL
