...
<Steps>
1. The `run` instruction is used to call the <code class="dark:text-orange-300 text-yellow-800">LOAD_TEST_DATA</code> subworkflow, under the alias
   <code class="dark:text-orange-300 text-yellow-800">LOAD_DATA</code>. The alias could be useful if multiple calls to the same subworkflow are needed.
   The `script` parameter inside the `run` block specifies the path to the <code class="dark:text-orange-300 text-yellow-800">LOAD_TEST_DATA</code>
   subworkflow file.
2. The `workflow` block contains definitions for the input channels of the subworkflow. The <code class="dark:text-orange-300 text-yellow-800">input[0]</code>
   channel corresponds to **the archives** to download for the test, while <code class="dark:text-orange-300 text-yellow-800">input[1]</code> is a
...
:::tip[What is happening here?]
<Steps>
1. The first input channel of the subworkflow is created from the output of the <code class="dark:text-orange-300 text-yellow-800">LOAD_TEST_DATA</code>
   subworkflow, called in the `setup` section, and accessed through its alias <code class="dark:text-orange-300 text-yellow-800">LOAD_DATA</code>.
