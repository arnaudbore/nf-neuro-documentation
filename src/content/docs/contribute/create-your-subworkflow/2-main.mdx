...
<Steps>
1. Each line in the <code class="dark:text-blue-300 text-blue-500">take</code> section define a distinct input channel users employ to pass data into your subworkflow.
2. The <code class="dark:text-blue-300 text-blue-500">DENOISING_NLMEANS</code> module optionally takes two masks in addition to an image to denoise. The `map` operator
...
3. The data prepared at the step above is given to the <code class="dark:text-blue-300 text-blue-500">DENOISING_NLMEANS</code> module. Then, the dependencies versions are
...
:::tip[What is happening here ?]
<Steps>
1. To prepare the input for synthbet, we access the output of <code class="dark:text-blue-300 text-blue-500">DENOISING_NLMEANS</code>, using its output map and the
   name of the channel of interest (here `image`, refer to the [API documentation](/api/modules/denoising/nlmeans)
   for more).
2. Just as previously, the <code class="dark:text-blue-300 text-blue-500">BETCROP_SYNTHBET</code> module is called on the prepared inputs and the versions collected.
</Steps>
:::
