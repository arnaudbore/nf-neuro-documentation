...
[**Part 3: Use and configure a nf-neuro module**](/pipeline/tutorial_step_3).
...
3. Configure the denosing module in the `nextflow.config` using the [API Documentation](/api/modules/denoising/mppca) as a reference.
...
4. Adapt the input for `RECONST_DTIMETRICS` module to attach it after the newly included denoised DWI module, reusing its outputs as inputs.
    ...
    <li>`DENOISING_MPPCA.out.image` : stores the denoised output.</li>
        ...
        The **name** of a module's **output(s)** can be determined via the [API Documentation](/api/categories) in the `Outputs` section.
    ...
    <li>`.map{ it + [[]] }`:  Adds an empty list `([])` to each input tuple.</li>
        ...
        for the optional mask input. The `map` operation is similar to the one we did for the DTI module in [**Part 2**](/pipeline/tutorial_step_2).
    </html>
    :::
</Steps>
