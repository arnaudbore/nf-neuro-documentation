...
which will display all subworkflows in the terminal.
   ...
   <span slot="output">
    ...
    INFO     Subworkflows available from https://github.com/nf-neuro/modules.git (main):                                                                                                          
...
To include a T1w image in the pipeline, we will follow the same approach from [**Part 2**](/pipeline/tutorial_step_2) as we did for the DWI images.
...
#### b. Prepare structure input for the subworkflow
    ...
    As with modules, the API documentation also lists [**subworkflows**](/api/categories), 
    providing information on **Inputs**, **Outputs** and the module list used in the 
    **Components** section:  [PREPROC_T1](/api/subworkflows/preproc_t1/).
...
The [`PREPROC_T1`](/api/subworkflows/preproc_t1/) API documentation 
